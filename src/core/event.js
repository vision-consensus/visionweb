...
export default class Event {
    ...
    setServer(eventServer = false, healthcheck = 'healthcheck') {
        if (!eventServer)
            return this.visionWeb.eventServer = false;

        if (utils.isString(eventServer))
            eventServer = new providers.HttpProvider(eventServer);

        if (!this.visionWeb.isValidProvider(eventServer))
            throw new Error('Invalid event server provided');

        this.visionWeb.eventServer = eventServer;
        this.visionWeb.eventServer.isConnected = () => this.visionWeb.eventServer.request(healthcheck).then(() => true).catch(() => false);
    }
    ...
    getEventsByContractAddress(contractAddress = false, options = {}, callback = false) {
        ...
        if (contractAddress)
            routeParams.push(this.visionWeb.address.fromHex(contractAddress));
        ...
        return this.visionWeb.eventServer.request(`event/contract/${routeParams.join('/')}?${querystring.stringify(qs)}`).then((data = false) => {
            if (!data)
    ...
    getEventsByTransactionID(transactionID = false, options = {}, callback = false) {
        ...
        return this.visionWeb.eventServer.request(`event/transaction/${transactionID}`).then((data = false) => {
            if (!data)
                return callback('Unknown error occurred');
