...
const VS_MESSAGE_HEADER = "\x19VISION Signed Message:\n32";
...
export default class Vs {
    ...
    _parseToken(token) {
        return {
            ...token,
            name: this.visionWeb.toUtf8(token.name),
            abbr: token.abbr && this.visionWeb.toUtf8(token.abbr),
            description:
                token.description && this.visionWeb.toUtf8(token.description),
            url: token.url && this.visionWeb.toUtf8(token.url),
        };
    }
    ...
    getBlock(block = this.visionWeb.defaultBlock, callback = false) {
        if (utils.isFunction(block)) {
            callback = block;
            block = this.visionWeb.defaultBlock;
        }
    ...
    getTransactionsToAddress(
        ...
        address = this.visionWeb.address.toHex(address);
    ...
    getAccount(address = this.visionWeb.defaultAddress.hex, callback = false) {
        if (utils.isFunction(address)) {
            callback = address;
            address = this.visionWeb.defaultAddress.hex;
        }
    ...
    getBalance(address = this.visionWeb.defaultAddress.hex, callback = false) {
        if (utils.isFunction(address)) {
            callback = address;
            address = this.visionWeb.defaultAddress.hex;
        }
    ...
    getTokenFromID(tokenID = false, callback = false) {
        ...
        this.visionWeb.fullNode
            .request(
                ...
                {
                    value: this.visionWeb.fromUtf8(tokenID),
                },
                "post"
            )
            .then((token) => {
        ...
        this.visionWeb.fullNode
            ...
            .then(({ nodes = [] }) => {
                callback(
                    ...
                    nodes.map(
                        ({ address: { host, port } }) =>
                            `${this.visionWeb.toUtf8(host)}:${port}`
                    )
                );
            })
            .catch((err) => callback(err));
    ...
    listTokens(limit = 0, offset = 0, callback = false) {
        ...
        if (!limit) {
            return this.visionWeb.fullNode
    ...
    getContract(contractAddress, callback = false) {
        ...
        contractAddress = this.visionWeb.address.toHex(contractAddress);
    ...
    async verifyMessage(
        message = false,
        signature = false,
        address = this.visionWeb.defaultAddress.base58,
        ...
        if (utils.isFunction(useVisionHeader)) {
        ...
        if (Vs.verifySignature(message, address, signature, useVisionHeader))
    ...
    static verifySignature(
        ...
        const messageBytes = [
            ...toUtf8Bytes(
                useVisionHeader ? VS_MESSAGE_HEADER : ETH_MESSAGE_HEADER
            ),
        ...
        const visionAddress = ADDRESS_PREFIX + recovered.substr(2);
        const base58Address = VisionWeb.address.fromHex(visionAddress);

        return base58Address == VisionWeb.address.fromHex(address);
    }
    ...
    async sign(
        ...
        if (utils.isString(transaction)) {
            ...
            try {
                const signatureHex = Vs.signString(
                    transaction,
                    privateKey,
                    useVisionHeader
                );
        ...
        try {
            if (!multisig) {
                const address = this.visionWeb.address
                    .toHex(this.visionWeb.address.fromPrivateKey(privateKey))
                    .toLowerCase();

                if (
                    address !==
                    this.visionWeb.address.toHex(
    ...
    static signString(message, privateKey, useVisionHeader = true) {
        message = message.replace(/^0x/, "");
        const value = {
    ...
    async sendTransaction(
        ...
        if (!this.visionWeb.isAddress(to))
        ...
        try {
            const address = options.privateKey
                ? this.visionWeb.address.fromPrivateKey(options.privateKey)
                : options.address;
            const transaction = await this.visionWeb.transactionBuilder.sendVs(
                to,
                amount,
                address
            );
    ...
    async sendToken(
        ...
        try {
            const address = options.privateKey
                ? this.visionWeb.address.fromPrivateKey(options.privateKey)
                : options.address;
            const transaction = await this.visionWeb.transactionBuilder.sendToken(
                to,
                amount,
                tokenID,
                address
    ...
    async freezeBalance(
        ...
        try {
            const address = options.privateKey
                ? this.visionWeb.address.fromPrivateKey(options.privateKey)
                : options.address;
            const freezeBalance = await this.visionWeb.transactionBuilder.freezeBalance(
                amount,
                duration,
                resource,
                address,
    ...
    async unfreezeBalance(
        ...
        try {
            const address = options.privateKey
                ? this.visionWeb.address.fromPrivateKey(options.privateKey)
                : options.address;
            const unfreezeBalance = await this.visionWeb.transactionBuilder.unfreezeBalance(
                resource,
                address,
                receiverAddress
            );
    ...
    async updateAccount(accountName = false, options = {}, callback = false) {
        ...
        try {
            const address = options.privateKey
                ? this.visionWeb.address.fromPrivateKey(options.privateKey)
                : options.address;
            const updateAccount = await this.visionWeb.transactionBuilder.updateAccount(
                accountName,
                address
            );
            const signedTransaction = await this.sign(
