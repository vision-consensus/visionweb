...
describe('VisionWeb.vs', function () {
    ...
    describe('#constructor()', function () {
        ...
        it('should have been set a full instance in visionWeb', function () {
            assert.instanceOf(visionWeb.vs, VisionWeb.Vs);
        });
    ...
    describe('#Account Test', function () {
        ...
        describe("#getAccount", async function () {
            ...
            it('should get account by hex or base58 address', async function () {
                ...
                for (let type of addressType) {
                    account = await visionWeb.vs.getAccount(accounts[type][idx]);
            ...
            it('should throw address is not valid error', async function () {
                await assertThrow(
                    visionWeb.vs.getAccount('notAnAddress'),
        ...
        describe("#getAccountById", async function () {
            ...
            before(async function(){
                this.timeout(10000);
                accountId = VisionWeb.toHex(`testtest${Math.ceil(Math.random()*100)}`);
                const transaction = await visionWeb.transactionBuilder.setAccountId(accountId, accounts.hex[idx]);
            ...
            it('should get confirmed account by id', async function () {
                this.timeout(20000);
                while (true) {
                    const account = await visionWeb.vs.getAccountById(accountId);
            ...
            it('should throw accountId is not valid error', async function () {
                ...
                for (let id of ids) {
                    await assertThrow(
                        visionWeb.vs.getAccountById(id),
        ...
        describe("#getAccountResources", async function () {
            ...
            it('should throw address is not valid error', async function () {
                await assertThrow(
                    visionWeb.vs.getAccountResources('notAnAddress'),
                    'Invalid address provided'
                );
            });
        ...
        describe("#getBalance", async function () {
            ...
            it('should get balance by hex or base58 address', async function () {
                ...
                let balance;
                for (let type of addressType) {
                    balance = await visionWeb.vs.getBalance(accounts[type][idx]);
        ...
        describe("#getUnconfirmedAccount", async function () {
            ...
            before(async function(){
                ...
                const account = await visionWeb.createAccount();
                toHex = account.address.hex;
                const transaction = await visionWeb.transactionBuilder.sendVs(account.address.hex, 10e5, accounts.hex[idx]);
                await broadcaster(null, accounts.pks[idx], transaction);
                await waitChainData('account', account.address.hex);
            });

            it('should get unconfirmed account by address', async function () {
                const account = await visionWeb.vs.getUnconfirmedAccount(toHex);
                assert.equal(account.address, toHex.toLowerCase());
            });

            it('should throw address is not valid error', async function () {
                await assertThrow(
                    visionWeb.vs.getUnconfirmedAccount('notAnAddress'),
                    'Invalid address provided'
                );
            });
        ...
        describe("#geUnconfirmedAccountById", async function () {
            ...
            it('should get unconfirmed account by id', async function () {
                ...
                const account = await visionWeb.vs.getUnconfirmedAccountById(accountId);
                assert.equal(account.account_id, accountId.slice(2));
            });
            ...
            it('should throw accountId is not valid error', async function () {
                ...
                for (let id of ids) {
                    await assertThrow(
                        visionWeb.vs.getUnconfirmedAccountById(id),
                        'Invalid accountId provided'
                    );
                }
            });
        ...
        describe("#getUnconfirmedBalance", async function () {
            ...
            it('should get unconfirmed balance by account address', async function () {
                const balance = await visionWeb.vs.getUnconfirmedBalance(toHex);
        ...
        describe("#updateAccount", async function () {
            ...
            it('should update account name', async function () {
                ...
                await visionWeb.vs.updateAccount(accountName, { privateKey: accounts.pks[idx], address: accounts.hex[idx] });
                const account = await visionWeb.vs.getUnconfirmedAccount(accounts.hex[idx]);
                assert.equal(visionWeb.toUtf8(account.account_name), accountName);
            });

            it('should throw name must be a string error', async function () {
                await assertThrow(
                    visionWeb.vs.updateAccount({}),
    ...
    describe('#Sign Test', function () {
        ...
        describe("#sign", async function () {
            ...
            it('should sign a transaction', async function () {
                const signedTransaction = await visionWeb.vs.sign(transaction, accounts.pks[idx]);
            ...
            it('should throw transaction not valid error', async function () {
                await assertThrow(
                    visionWeb.vs.sign(undefined, accounts.pks[idx]),
            ...
            it('should throw transaction is already signed error', async function () {
                const signedTransaction = await visionWeb.vs.sign(transaction, accounts.pks[idx]);
                await assertThrow(
                    visionWeb.vs.sign(signedTransaction, accounts.pks[idx]),
            ...
            it('should throw private key does not match address error', async function () {
                await assertThrow(
                    visionWeb.vs.sign(transaction, accounts.pks[idx]),
        ...
        describe("#signMessage", async function () {
            ...
            it('should sign a hex string message', async function () {
                ...
                const signedMsg = await visionWeb.vs.sign(hexMsg, accounts.pks[idx]);
            ...
            it('should throw expected hex message input error', async function () {
                ...
                await assertThrow(
                    visionWeb.vs.sign(hexMsg, accounts.pks[idx]),
                    'Private key does not match address in transaction'
                );
            });
        ...
        describe("#verifyMessage", async function () {
            ...
            before(async function() {
                ...
                signedMsg = await visionWeb.vs.sign(hexMsg, accounts.pks[idx], null, false);
            });

            it('should verify signature of signed string message', async function () {
                const result = await visionWeb.vs.verifyMessage(hexMsg, signedMsg, accounts.hex[idx], null);
                assert.isTrue(result);
            });

            it('should throw expected hex message input error', async function () {
                await assertThrow(
                    visionWeb.vs.verifyMessage('e66f4c8f323229131006ad3e4a2ca65dfdf339f0', signedMsg, accounts.hex[idx], null),
            ...
            it('should throw signature does not match error', async function () {
                ...
                await assertThrow(
                    visionWeb.vs.verifyMessage(hexMsg, fakeSig, accounts.hex[idx], null),
        ...
        describe("#multiSignTransaction", async function () {
            ...
            before(async function() {
                ...
                const updateTransaction = await visionWeb.transactionBuilder.updateAccountPermissions(
                    ownerAddress,
                    ownerPermission,
                    null,
                    [activePermission]
                ...
                const signedUpdateTransaction = await visionWeb.vs.sign(updateTransaction, ownerPk, null, false);
                await visionWeb.vs.broadcast(signedUpdateTransaction);
            ...
            it('should multi-sign a transaction by owner permission', async function () {
                ...
                const result = await visionWeb.vs.broadcast(signedTransaction);
            ...
            it('should verify weight after multi-sign by owner permission', async function () {
                ...
                for (let i = idxS; i < idxE; i++) {
                    signedTransaction = await visionWeb.vs.multiSign(signedTransaction, accounts.pks[i], 0);
                    signWeight = await visionWeb.vs.getSignWeight(signedTransaction);
                    if (i < idxE - 1) {
                ...
                // get approved list
                const approvedList = await visionWeb.vs.getApprovedList(signedTransaction);
            ...
            it('should verify weight after multi-sign by active permission', async function () {
                ...
                for (let i = idxS; i < idxE; i++) {
                    signedTransaction = await visionWeb.vs.multiSign(signedTransaction, accounts.pks[i], 2);
                    signWeight = await visionWeb.vs.getSignWeight(signedTransaction, 2);
                    if (i < idxE - 1) {
                        assert.equal(signWeight.result.code, 'NOT_ENOUGH_PERMISSION');
                    }
                    assert.equal(signWeight.approved_list.length, i - idxS + 1);
    ...
    describe('#Block Test', function () {
        ...
        describe("#getBlock", async function () {
            ...
            it('should get earliest or latest block', async function () {
                ...
                for (let type of blockType) {
                    block = await visionWeb.vs.getBlock(type);
                    if (type === 'earliest') {
        ...
        describe("#getBlockByHash", async function () {
            ...
            it('should get block by block hash (id)', async function () {
                const block = await visionWeb.vs.getBlock('latest');
                const blockByHash = await visionWeb.vs.getBlockByHash(block.blockID);
        ...
        describe("#getBlockByNumber", async function () {
            ...
            it('should get block by block number', async function () {
                const block = await visionWeb.vs.getBlock('latest');
                const blockByNumber = await visionWeb.vs.getBlockByNumber(block.block_header.raw_data.number);
        ...
        describe("#getBlockRange", async function () {
            ...
            it('should get block by range', async function () {
                const blocks = await visionWeb.vs.getBlockRange(0, 5);
            ...
            it('should get invalid start or end error by range', async function () {
                ...
                for (let range of ranges) {
                    await assertThrow(
                        visionWeb.vs.getBlockRange(range[0], range[1]),
        ...
        describe("#getBlockTransactionCount", async function () {
            ...
            it('should get transaction count by block number, \'latest\' or \'earliest\'', async function () {
                ...
                for (let type of blockType) {
                    const count = await visionWeb.vs.getBlockTransactionCount(type);
        ...
        describe("#getCurrentBlock", async function () {
            ...
            it('should get current block', async function () {
                const block = await visionWeb.vs.getCurrentBlock();
                assert.isNumber(block.block_header.raw_data.number);
            });
    ...
    describe('#Transaction Test', function () {
        ...
        describe("#send", async function () {
            ...
            it('should send vs', async function () {
                this.timeout(10000);

                const balanceBefore = await visionWeb.vs.getUnconfirmedBalance(accounts.hex[toIdx]);
                await visionWeb.vs.send(accounts.hex[toIdx], 10e5, { privateKey: accounts.pks[fromIdx], address: accounts.hex[fromIdx] });
                await waitChainData('balance', accounts.hex[toIdx], balanceBefore);
                const balanceAfter = await visionWeb.vs.getUnconfirmedBalance(accounts.hex[toIdx]);
                assert.equal(balanceAfter - balanceBefore, 10e5);
            });

            it('should throw invalid recipient provided error', async function () {
                await assertThrow(
                    visionWeb.vs.send('notValidAddress', 10e5, { privateKey: accounts.pks[fromIdx] }),
            ...
            it('should throw invalid amount provided error', async function () {
                await assertThrow(
                    visionWeb.vs.send(accounts.hex[toIdx], -1, { privateKey: accounts.pks[fromIdx] }),
        ...
        describe("#sendTransaction", async function () {
            ...
            it('should send vs', async function () {
                ...
                const balanceBefore = await visionWeb.vs.getUnconfirmedBalance(accounts.hex[toIdx]);
                await visionWeb.vs.sendTransaction(accounts.hex[toIdx], 10e5, { privateKey: accounts.pks[fromIdx] });
                await waitChainData('balance', accounts.hex[toIdx], balanceBefore);
                const balanceAfter = await visionWeb.vs.getUnconfirmedBalance(accounts.hex[toIdx]);
                assert.equal(balanceAfter - balanceBefore, 10e5);
            });
            ...
            it('should throw invalid recipient provided error', async function () {
                await assertThrow(
                    visionWeb.vs.sendTransaction('notValidAddress', 10e5, { privateKey: accounts.pks[fromIdx] }),
                    'Invalid recipient provided'
                );
            });
            ...
            it('should throw invalid amount provided error', async function () {
                await assertThrow(
                    visionWeb.vs.sendTransaction(accounts.hex[toIdx], -1, { privateKey: accounts.pks[fromIdx] }),
                    'Invalid amount provided'
                );
            });
        ...
        describe("#sendVs", async function () {
            ...
            it('should send vs', async function () {
                ...
                const balanceBefore = await visionWeb.vs.getUnconfirmedBalance(accounts.hex[toIdx]);
                await visionWeb.vs.sendVs(accounts.hex[toIdx], 10e5, { privateKey: accounts.pks[fromIdx], address: accounts.hex[fromIdx] });
                await waitChainData('balance', accounts.hex[toIdx], balanceBefore);
                const balanceAfter = await visionWeb.vs.getUnconfirmedBalance(accounts.hex[toIdx]);
                assert.equal(balanceAfter - balanceBefore, 10e5);
            });
            ...
            it('should throw invalid recipient provided error', async function () {
                await assertThrow(
                    visionWeb.vs.sendVs('notValidAddress', 10e5, { privateKey: accounts.pks[fromIdx] }),
                    'Invalid recipient provided'
                );
            });
            ...
            it('should throw invalid amount provided error', async function () {
                await assertThrow(
                    visionWeb.vs.sendVs(accounts.hex[18], -1, { privateKey: accounts.pks[fromIdx] }),
                    'Invalid amount provided'
                );
            });
        ...
        */
        ...
        describe("#broadcast", async function () {
            ...
            it('should throw invalid options provided error', async function () {
                await assertThrow(
                    visionWeb.vs.broadcast(signedTransaction, false),
            ...
            it('should throw transaction is not signed error', async function () {
                await assertThrow(
                    visionWeb.vs.broadcast(transaction),
        ...
        describe("#getTransaction", async function () {
            ...
            it('should get transaction by id', async function () {
                const tx = await visionWeb.vs.getTransaction(transaction.txID);
                assert.equal(tx.txID, transaction.txID);
            });

            it('should throw transaction not found error', async function () {
                await assertThrow(
                    visionWeb.vs.getTransaction('a8813981b1737d9caf7d51b200760a16c9cdbc826fa8de102386af898048cbe5'),
        ...
        describe("#getTransactionFromBlock", async function () {
            ...
            it('should get transaction from block', async function () {
                ...
                for (let i = currBlockNum; i < currBlockNum + 3;) {
                    try {
                        const tx = await visionWeb.vs.getTransactionFromBlock(i, 0);
                        // assert.equal(tx.txID, transaction.txID);
            ...
            it('should throw transaction not found error by transaction from block', async function () {
                await assertThrow(
                    visionWeb.vs.getTransactionFromBlock(currBlockNum - 1, 0),
            ...
            it('should throw block not found error by transaction from block', async function () {
                await assertThrow(
                    visionWeb.vs.getTransactionFromBlock(currBlockNum + 50, 0),
                    'Block not found'
                );
            });
            ...
            it('should throw invalid index error by transaction from block', async function () {
                await assertThrow(
                    visionWeb.vs.getTransactionFromBlock(currBlockNum, -1),
        ...
        describe("#geUnconfirmedTransactionInfo", async function () {
            ...
            it('should get unconfirmed transaction by id', async function () {
                this.timeout(10000)
                await wait(3)
                const tx = await visionWeb.vs.getUnconfirmedTransactionInfo(transaction.txID);
                assert.equal(tx.id, transaction.txID);
            });

            it('should throw transaction not found error', async function () {
                await assertThrow(
                    visionWeb.vs.getUnconfirmedTransactionInfo('a8813981b1737d9caf7d51b200760a16c9cdbc826fa8de102386af898048cbe5'),
                    'Transaction not found'
                );
            });
        ...
        describe("#getConfirmedTransaction", async function () {
            ...
            it('should get confirmed transaction by tx id', async function () {
                ...
                while (true) {
                    try {
                        const tx = await visionWeb.vs.getConfirmedTransaction(transaction.transaction.txID);
    ...
    describe('#Token Test', function () {
        ...
        describe("#sendAsset", async function () {
            ...
            before(async function(){
                ...
                const transaction = await visionWeb.transactionBuilder.createToken(options, accounts.hex[fromIdx]);
                await broadcaster(null, accounts.pks[fromIdx], transaction);
                await waitChainData('token', accounts.hex[fromIdx]);
                token = await visionWeb.vs.getTokensIssuedByAddress(accounts.hex[fromIdx]);
            });

            it('should send vs by to address and verify account balance', async function () {
                this.timeout(20000);

                const assetBefore = (await visionWeb.vs.getUnconfirmedAccount(accounts.hex[toIdx])).assetV2;
                await waitChainData('tokenById', token[Object.keys(token)[0]]['id']);
                await visionWeb.vs.sendAsset(
                    accounts.hex[toIdx],
                    10e4,
                ...
                const assetAfter = (await visionWeb.vs.getUnconfirmedAccount(accounts.hex[toIdx])).assetV2;
                assert.equal(!assetBefore ? 0 : assetBefore[0].value, assetAfter[0].value - 10e4);
            });

            it('should throw invalid recipient provided error', async function () {
                await assertThrow(
                    visionWeb.vs.sendAsset(
                        'notValidAddress',
                        10e4,
                        token[Object.keys(token)[0]]['id'],
                        { privateKey: accounts.pks[fromIdx], address: accounts.hex[fromIdx] }
        ...
        describe("#getTokenFromID", async function () {
            ...
            before(async function(){
                ...
                const options = getTokenOptions();
                const transaction = await visionWeb.transactionBuilder.createToken(options, accounts.hex[idx]);
                await broadcaster(null, accounts.pks[idx], transaction);
                await waitChainData('token', accounts.hex[idx]);
            });

            it('should get token by name', async function () {
                const tokens = await visionWeb.vs.listTokens(5, 0);
                for (let token of tokens) {
                    const tk = await visionWeb.vs.getTokenFromID(token.id);
            ...
            it('should throw token does not exist error', async function () {
                await assertThrow(
                    visionWeb.vs.getTokenFromID(1234565),
        ...
        describe("#getTokensIssuedByAddress", async function () {
            ...
            it('should get token by issued address', async function () {
                const tokens = await visionWeb.vs.listTokens(5, 0);
                for (let token of tokens) {
                    const tk = await visionWeb.vs.getTokensIssuedByAddress(token.owner_address);
            ...
            it('should throw invalid address provided error', async function () {
                await assertThrow(
                    visionWeb.vs.getTokensIssuedByAddress('abcdefghijklmn'),
                    'Invalid address provided'
                );
            });
        ...
        describe("#listTokens", async function () {
            ...
            it('should list all tokens by limit', async function () {
                const tokens = await visionWeb.vs.listTokens(10, 0);
        ...
        describe("#parseToken", async function () {
            ...
            it('should list all tokens by limit', async function () {
                ...
                for (let token of tokens) {
                    ...
                    token.name = visionWeb.fromUtf8(token.name);
                    token.abbr = visionWeb.fromUtf8(token.abbr);
                    token.description = token.description && visionWeb.fromUtf8(token.description);
                    token.url = visionWeb.fromUtf8(token.url);

                    const tk = visionWeb.vs._parseToken(token);
    ...
    describe('#Exchange Test', function () {
        ...
        describe("#listExchanges", async function () {
            ...
            before(async function(){
                ...
                await broadcaster(
                    null,
                    accounts.pks[toIdx],
                    await visionWeb.transactionBuilder.createTokenExchange(tokenNames[0], 10e3, tokenNames[1], 10e3, accounts.hex[toIdx])
                );

            });

            it('should get exchange by id', async function () {
                const exchanges = await visionWeb.vs.listExchanges();
        ...
        describe("#listExchangesPaginated", async function () {
            ...
            it('should get exchange by id', async function () {
                const exchanges = await visionWeb.vs.listExchangesPaginated(10, 0);
                assert.isArray(exchanges);
        ...
        describe("#getExchangeByID", async function () {
            ...
            before(async function(){
                ...
                for (let i = idxS; i < idxE; i++) {
                    const options = getTokenOptions();
                    await broadcaster(null, accounts.pks[i], await visionWeb.transactionBuilder.createToken(options, accounts.hex[i]));
                    await waitChainData('token', accounts.hex[i]);
                    const token = await visionWeb.vs.getTokensIssuedByAddress(accounts.hex[i]);
                    await waitChainData('tokenById', token[Object.keys(token)[0]]['id']);
                    await broadcaster(null, accounts.pks[i], await visionWeb.transactionBuilder.sendToken(
                ...
                exchanges = await visionWeb.vs.listExchanges();
            });

            it('should get exchange by id', async function () {
                for (let exchange of exchanges) {
                    const ex = await visionWeb.vs.getExchangeByID(exchange.exchange_id);
    ...
    describe("#Proposal Test", async function () {
        ...
        describe("#getChainParameters", async function () {
            ...
            it('should get proposal list', async function () {
                const params = await visionWeb.vs.getChainParameters();
        ...
        describe("#getProposal", async function () {
            ...
            before(async function(){
                ...
                await broadcaster(
                    null,
                    PRIVATE_KEY,
                    await visionWeb.transactionBuilder.createProposal(parameters[0], ADDRESS_BASE58)
                );
            ...
            it('should get proposal by id', async function () {
                for (let proposal of proposals) {
                    const ps = await visionWeb.vs.getProposal(proposal.proposal_id);
        ...
        describe("#listProposals", async function () {
            ...
            it('should list seeds node', async function () {
                const proposals = await visionWeb.vs.listProposals();
                for (let proposal of proposals) {
    ...
    describe("#getContract", async function () {
        ...
        it('should get contract by contract address', async function () {
            const contract = await visionWeb.vs.getContract(transaction.contract_address);
        ...
        it('should throw invalid contract address provided error', async function () {
            await assertThrow(
                visionWeb.vs.getContract('notAddress'),
        ...
        it('should throw contract does not exist error', async function () {
            await assertThrow(
                visionWeb.vs.getContract('417cbcc41052b59584d1ac9fc1ce39106533aa1d40'),
    ...
    describe("#listNodes", async function () {
        ...
        it('should list seeds node', async function () {
            const nodes = await visionWeb.vs.listNodes();
    ...
    describe("#listSuperRepresentatives", async function () {
        ...
        it('should list super representatives', async function () {
            const srs = await visionWeb.vs.listSuperRepresentatives();
            assert.isArray(srs);
            for (let sr of srs) {
    ...
    describe("#timeUntilNextVoteCycle", async function () {
        ...
        it('should get time util next vote cycle', async function () {
            const num = await visionWeb.vs.timeUntilNextVoteCycle();
            assert.isNumber(num);
        });
    ...
    describe("#getReward", async function () {
        it('should get the reward', async function () {
            ...
            let reward = await visionWeb.vs.getReward(accounts[0]);
    ...
    describe("#getUnconfirmedReward", async function () {
        it('should get the reward', async function () {
            ...
            let reward = await visionWeb.vs.getUnconfirmedReward(accounts[0]);
            assert.equal(reward, 0)
    ...
    describe("#getBrokerage", async function () {
        it('should get the brokerage', async function () {
            ...
            let brokerage = await visionWeb.vs.getBrokerage(accounts[0]);
    ...
    describe("#getUnconfirmedBrokerage", async function () {
        it('should get the brokerage', async function () {
            ...
            let brokerage = await visionWeb.vs.getUnconfirmedBrokerage(accounts[0]);
            assert.equal(brokerage, 20)
