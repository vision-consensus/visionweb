...
describe("VisionWeb.core.event", async function () {
    let accounts;
    let visionWeb;
    let contractAddress;
    let contract;
    ...
    before(async function () {
        ...
        const result = await broadcaster(
            visionWeb.transactionBuilder.createSmartContract(
                {
                    abi: [
                        {
                            anonymous: false,
    ...
    describe("#constructor()", function () {
        it("should have been set a full instance in visionWeb", function () {
            assert.instanceOf(visionWeb.event, VisionWeb.Event);
        });
    });
    ...
    describe("#getEventsByTransactionID", async function () {
        it("should emit an unconfirmed event and get it", async function () {
            ...
            let events;
            while (true) {
                events = await visionWeb.event.getEventsByTransactionID(txId);
                if (events.length) {
                    break;
                }
                await wait(0.5);
    ...
    describe("#getEventsByContractAddress", async function () {
        it("should emit an event and wait for it", async function () {
            ...
            let events;
            while (true) {
                events = await visionWeb.event.getEventsByContractAddress(
                    contractAddress,
                    {
                        eventName: "SomeEvent",
